"""Run each exercise's tests and publish a five-point grading summary."""
...
EXERCISES = (
    ("Test 00_01", "test_00_01"),
    ("Test 01_01", "test_01_01"),
    ("Test 01_02", "test_01_02"),
    ("Test 01_03", "test_01_03"),
    ("Test 01_04", "test_01_04"),
)
...
def run_exercise(name: str, selector: str) -> dict[str, object]:
    command = [sys.executable, "-m", "pytest", "-q", "-k", selector]
...
def main() -> int:
    results = [run_exercise(name, selector) for name, selector in EXERCISES]
    points = sum(int(result["points"]) for result in results)
    max_points = sum(int(result["max_points"]) for result in results)
